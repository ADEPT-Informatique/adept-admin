...
if (!($id && $size && !is_nan($depot))){
    header('Location: '.url_for(dirname(dirname(__DIR__)).'/hoodies/detailsReservation.php').'?update=fail&id='.$id);
    die();
}
...
if($depot < 0 || $depot > 40){
    header('Location: '.url_for(dirname(dirname(__DIR__)).'/hoodies/detailsReservation.php').'?update=fail&id='.$id.'&depot='.$depot);
    die();
}

if ($isrecupered && $depot<40){
    header('Location: '.url_for(dirname(dirname(__DIR__)).'/hoodies/detailsReservation.php').'?update=fail-invalidrecup&id='.$id);
    die();
}
...
header('Location: '.url_for(dirname(dirname(__DIR__)).'/hoodies/detailsReservation.php').'?update=success&id='.$id);
